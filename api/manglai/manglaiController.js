...
module.exports.fetchBuildings = async (req, res) => {
  const companyId = req.params.companyId;
  const { startDate, endDate } = req.query;
...
module.exports.fetchConsumptions = async (req, res) => {
  const companyId = req.params.companyId;
  const { startDate, endDate } = req.query;
...
module.exports.fetchConsumptionById = async (req, res) => {
  try {
...
module.exports.fetchEmissions = async (req, res) => {
  const companyId = req.params.companyId;
  try {
...
module.exports.fetchEmissionsDashboard = async (req, res) => {
  const companyId = req.params.companyId;
...
module.exports.fetchVehicles = async (req, res) => {
  const companyId = req.params.companyId;
  const { startDate, endDate } = req.query;

  try {
    const [vehiclesData, dashboardData] = await Promise.all([
    ...
    console.error("fetchVehiclesAndDashboard error: ", error);
    return res.status(500).json({ error: error.message });
  }
};

module.exports.fetchVehicleById = async (req, res) => {
  try {
    const { id, companyId } = req.params;
